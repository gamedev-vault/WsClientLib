...
namespace { // private module-only namespace
	...
	class BaseSocket
	{
		...
		protected:
			...
			void set_errstr(stlimpl::string e)
			{
				error_string = e;
			}
		...
		public:
			...
			stlimpl::string get_error_string() { return error_string; }
			virtual ~BaseSocket() {}
	};
	...
	class _RealWebSocket : public WsClientLib::WebSocket
	{
	public:
		...
		stlimpl::vector<char> rxbuf;
		stlimpl::vector<char> txbuf;

		readyStateValues readyState;
        bool useMask;
		BaseSocket* socket;
                
        _RealWebSocket(stlimpl::string host, stlimpl::string path, int port, stlimpl::string url, stlimpl::string origin, bool _useMask, bool _useSSL)
        {
            m_host = host;
            m_path = path;
            m_port = port;
		...
		void poll(int timeout, WSErrorCallback errorCallback, void* userData)  // timeout in milliseconds
        {
            ...
            {           
                ...
                for(;;) // while(true), but without a warning about constant expression
                {
                    ...
                    stlimpl::vector<char>::size_type N = rxbuf.size();
					rxbuf.resize(N + 1500);
                    ...
                    {
                        rxbuf.resize(static_cast<stlimpl::vector<char>::size_type>(N + ret));
                    }
                }
		...
		// Callable must have signature: void(const stlimpl::string & message).
		...
		virtual void _dispatch(WSMessageCallback messageCallback, WSErrorCallback errorCallback, void* userData)
        {
			...
			for(;;) // while (true) withoput warning about constant expression
            {
				...
				if (ws.opcode == wsheader_type::TEXT_FRAME && ws.fin) {
					if (ws.mask) { for (size_t i = 0; i != ws.N; ++i) { rxbuf[static_cast<stlimpl::vector<char>::size_type>(i+ws.header_size)] ^= ws.masking_key[i&0x3]; } }
					stlimpl::string data(rxbuf.begin()+ws.header_size, rxbuf.begin()+ws.header_size+(size_t)ws.N);
					messageCallback((const stlimpl::string) data, userData);
				}
                ...
                {
					messageCallback((const stlimpl::string) "{ \"@class\" : \".pong\" }", userData);
                }
        ...
        {
            if(readyState == CONNECTING) return;
        	sendData(wsheader_type::PING, stlimpl::string());
		}

		void send(const stlimpl::string& message)
        {
            if(readyState == CONNECTING) return;
			sendData(wsheader_type::TEXT_FRAME, message);
		}

		void sendData(wsheader_type::opcode_type type, const stlimpl::string& message)
        {
			// TODO:
		...
		void close() {
			...
			stlimpl::vector<uint8_t> header(closeFrame, closeFrame+6);
			txbuf.insert(txbuf.end(), header.begin(), header.end());
		}
	...
	WsClientLib::WebSocket::pointer from_url(const stlimpl::string& url, bool useMask, const stlimpl::string& origin)
    {
		char host[256];
		int port;
		char path[256];
		...
		fprintf(stderr, "WsClientLib: connecting: host=%s port=%d path=/%s\n", host, port, path);
        
        _RealWebSocket *nWebsocket = new _RealWebSocket(host, path, port, url, origin, useMask, secure_connection);
        
        return WsClientLib::WebSocket::pointer(nWebsocket);
	}
...
namespace WsClientLib {
	WebSocket::pointer WebSocket::from_url(const stlimpl::string& url, const stlimpl::string& origin) {
		return ::from_url(url, true, origin);
	}

	WebSocket::pointer WebSocket::from_url_no_mask(const stlimpl::string& url, const stlimpl::string& origin) {
